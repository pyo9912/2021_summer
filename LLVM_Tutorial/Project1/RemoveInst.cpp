	...
	for( llvm::Module::iterator ModIter = TheModule->begin(); ModIter != TheModule->end(); ++ModIter )
	{
		...
		for( llvm::Function::iterator FuncIter = Func->begin(); FuncIter != Func->end(); ++FuncIter )
		{
			...
			for( llvm::BasicBlock::iterator BBIter = BB->begin(); BBIter != BB->end(); ++BBIter )
			{
				...
				llvm::Instruction* LwInst = llvm::cast<llvm::Instruction>(BBIter);
				BBIter++;
				llvm::Instruction* OpInst = llvm::cast<llvm::Instruction>(BBIter);
				BBIter--;
				if( llvm::isa< llvm::LoadInst >( LwInst ))
				{
					if ( LwInst )
					{
						if( OpInst->getOpcode() == llvm::Instruction::Add )
						{
							llvm::ConstantInt* op0 = llvm::dyn_cast<llvm::ConstantInt>(OpInst->getOperand(0));
							llvm::ConstantInt* op1 = llvm::dyn_cast<llvm::ConstantInt>(OpInst->getOperand(1));
							if( op0->getSExtValue() == 0 || op1->getSExtValue() == 0 ) {
								InstsToDelete.push_back(OpInst);
								OpInst->replaceAllUsesWith(OpInst->getOperand(0));
							}
						}
						else if( OpInst->getOpcode() == llvm::Instruction::Mul )
						{
							llvm::ConstantInt* op0 = llvm::dyn_cast<llvm::ConstantInt>(OpInst->getOperand(0));
							llvm::ConstantInt* op1 = llvm::dyn_cast<llvm::ConstantInt>(OpInst->getOperand(1));
							if( op0->getSExtValue() == 1 || op1->getSExtValue() == 1 ) {
								InstsToDelete.push_back(OpInst);
								OpInst->replaceAllUsesWith(OpInst->getOperand(0));
							}
						}
						else if( OpInst->getOpcode() == llvm::Instruction::FMul )
						{
							llvm::ConstantInt* op0 = llvm::dyn_cast<llvm::ConstantInt>(OpInst->getOperand(0));
							llvm::ConstantInt* op1 = llvm::dyn_cast<llvm::ConstantInt>(OpInst->getOperand(1));
							if( op0->getSExtValue() == 1.0f || op1->getSExtValue() == 1.0f ) {
								InstsToDelete.push_back(OpInst);
								OpInst->replaceAllUsesWith(OpInst->getOperand(0));
							}
						}
			...
			for( int i=0, Size=InstsToDelete.size(); i<Size; ++i ) InstsToDelete[i]->eraseFromParent();
		}
	}
}
