...
void TraverseModule(void)
{
	for( llvm::Module::iterator ModIter = TheModule->begin(); ModIter != TheModule->end(); ++ModIter )
	{
		...
		for( llvm::Function::iterator FuncIter = Func->begin(); FuncIter != Func->end(); ++FuncIter )
		{
			...
			std::vector<llvm::Instruction*> LwInsts;

			for( llvm::BasicBlock::iterator BBIter = BB->begin(); BBIter != BB->end(); ++BBIter )
			{
				llvm::Instruction* SwInst = llvm::cast<llvm::Instruction>(BBIter);
				BBIter++;
				llvm::Instruction* LwInst = llvm::cast<llvm::Instruction>(BBIter);
				BBIter--;

				//if( Inst->getOpcode() == llvm::Instruction::Store )
				if( llvm::isa< llvm::StoreInst >( SwInst ) 
					&& llvm::isa< llvm::LoadInst >( LwInst )
					&& SwInst->getOperand(1) == LwInst->getOperand(0))
				{
					llvm::ConstantInt* Zero = llvm::ConstantInt::get( llvm::IntegerType::get( *TheContext, 32 ), 0, true );
					llvm::Instruction* TmpInst = llvm::BinaryOperator::Create(
					...
					LwInst->replaceAllUsesWith(TmpInst);
					LwInsts.push_back(LwInst);
				}
				
			}
			for(int i=0, Size=LwInsts.size(); i<Size; ++i) LwInsts[i]->eraseFromParent();
		}
	}
}
